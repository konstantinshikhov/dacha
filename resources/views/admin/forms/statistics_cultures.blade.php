@extends('admin.main')

@section('form')

<!-- klumba statistics -->
<div class="row">
  <div class="col-md-12 col-sm-12 col-xs-12">
    <div class="info-box">
      <span class="info-box-icon" style="background-color: #a95f8a;">
        <img src="data:image/png;base64," alt="Клумба">
      </span>
      <div class="info-box-content">
        <span class="info-box-text">Количество культур секции "Клумба"</span>
        <span class="info-box-number">{{ $klumba_cultures_count }}</span>
      </div>
    </div>
  </div>
</div>

<div class="row">
  <div class="col-md-6">
    <div class="box">
      <div class="box-header with-border text-center">
        <h3 class="box-title">Соотношение культур по типу области выращивания</h3>
      </div>
      <div class="box-body">
        <div class="chart">
          <canvas id="klumba_cultures_chart_bar_type" width="300" height="125"></canvas>
        </div>
      </div>
    </div>
  </div>

  <div class="col-md-6">
    <div class="box">
      <div class="box-header with-border text-center">
        <h3 class="box-title">Соотношение культур по назначению</h3>
      </div>
      <div class="box-body">
        <div class="chart">
          <canvas id="klumba_cultures_chart_bar_target" width="300" height="125"></canvas>
        </div>
      </div>
    </div>
  </div>
</div>
<!-- End klumba statistics -->
<hr>
<!-- Ogorod statistics -->
<div class="row">
  <div class="col-md-12 col-sm-12 col-xs-12">
    <div class="info-box">
      <span class="info-box-icon" style="background-color: #f39a24;">
        <img src="data:image/png;base64," alt="Огород">
      </span>
      <div class="info-box-content">
        <span class="info-box-text">Количество культур секции "Огород"</span>
        <span class="info-box-number">{{ $ogorod_cultures_count }}</span>
      </div>
    </div>
  </div>
</div>

<div class="row">
  <div class="col-md-12">
    <div class="box">
      <div class="box-header with-border text-center">
        <h3 class="box-title">Соотношение культур по морфологическому признаку</h3>
      </div>
      <div class="box-body">
        <div class="chart">
          <canvas id="ogorod_cultures_chart_bar_morph" width="300" height="75"></canvas>
        </div>
      </div>
    </div>
  </div>
</div>
<!-- End Ogorod statistics -->
<hr>
<!-- Sad statistics -->
<div class="row">
  <div class="col-md-12 col-sm-12 col-xs-12">
    <div class="info-box">
      <span class="info-box-icon" style="background-color: #179e36;">
        <img src="data:image/png;base64," alt="Сад">
      </span>
      <div class="info-box-content">
        <span class="info-box-text">Количество культур секции "Сад"</span>
        <span class="info-box-number">{{ $sad_cultures_count }}</span>
      </div>
    </div>
  </div>
</div>

<div class="row">
  <div class="col-md-6">
    <div class="box">
      <div class="box-header with-border text-center">
        <h3 class="box-title">Соотношение культур по типу плода</h3>
      </div>
      <div class="box-body">
        <div class="chart">
          <canvas id="sad_cultures_chart_bar_type" width="300" height="125"></canvas>
        </div>
      </div>
    </div>
  </div>

  <div class="col-md-6">
    <div class="box">
      <div class="box-header with-border text-center">
        <h3 class="box-title">Соотношение культур по морфологическому признаку</h3>
      </div>
      <div class="box-body">
        <div class="chart">
          <canvas id="sad_cultures_chart_bar_morph" width="300" height="125"></canvas>
        </div>
      </div>
    </div>
  </div>
</div>
<!-- End sad statistics -->

@endsection

@section('script')
@parent
<script src="{{ asset('adminlte/asset/Chart.min.js') }}"></script>
<script type="text/javascript">
$(document).ready(function() {
  /* Klumba statistics */
  var klumba_cultures_chart_bar_type_ctx = document.getElementById("klumba_cultures_chart_bar_type");
  var klumba_cultures_chart_bar_target_ctx = document.getElementById("klumba_cultures_chart_bar_target");
  var klumba_cultures_chart_bar_type_chart;
  var klumba_cultures_chart_bar_target_chart;

  klumba_cultures_chart_bar_type_chart = new Chart(klumba_cultures_chart_bar_type_ctx, {
    type: 'horizontalBar',
    data: {
      datasets: [{
        data: [@foreach($klumba_cultures_count_filters['type'] as $count){{ $count }},@endforeach],
        backgroundColor: '#a95f8a'
      }],
      labels: [@foreach($klumba_cultures_count_filters['type'] as $type => $count)'{{ $type }}',@endforeach]
    },
    options: {
      legend: {
        display: false
      },
      scales: {
        xAxes: [{ticks: {beginAtZero:true}}]
      }
    },
  });
  klumba_cultures_chart_bar_target_chart = new Chart(klumba_cultures_chart_bar_target_ctx, {
    type: 'horizontalBar',
    data: {
      datasets: [{
        data: [@foreach($klumba_cultures_count_filters['target'] as $count){{ $count }},@endforeach],
        backgroundColor: '#a95f8a'
      }],
      labels: [@foreach($klumba_cultures_count_filters['target'] as $type => $count)'{{ $type }}',@endforeach]
    },
    options: {
      legend: {
        display: false
      },
      scales: {
        xAxes: [{ticks: {beginAtZero:true}}]
      }
    },
  });
  /* End klumba statistics */

  /* Ogorod statistics */
  var ogorod_cultures_chart_bar_morph_ctx = document.getElementById("ogorod_cultures_chart_bar_morph");
  var ogorod_cultures_chart_bar_morph_chart;

  ogorod_cultures_chart_bar_morph_chart = new Chart(ogorod_cultures_chart_bar_morph_ctx, {
    type: 'horizontalBar',
    data: {
      datasets: [{
        data: [@foreach($ogorod_cultures_count_filters['morph'] as $count){{ $count }},@endforeach],
        backgroundColor: '#f39a24'
      }],
      labels: [@foreach($ogorod_cultures_count_filters['morph'] as $type => $count)'{{ $type }}',@endforeach]
    },
    options: {
      legend: {
        display: false
      },
      scales: {
        xAxes: [{ticks: {beginAtZero:true}}]
      }
    },
  });
  /* End ogorod statistics */

  /* Sad statistics */
  var sad_cultures_chart_bar_type_ctx = document.getElementById("sad_cultures_chart_bar_type");
  var sad_cultures_chart_bar_morph_ctx = document.getElementById("sad_cultures_chart_bar_morph");
  var sad_cultures_chart_bar_type_chart;
  var sad_cultures_chart_bar_morph_chart;

  sad_cultures_chart_bar_type_chart = new Chart(sad_cultures_chart_bar_type_ctx, {
    type: 'horizontalBar',
    data: {
      datasets: [{
        data: [@foreach($sad_cultures_count_filters['type'] as $count){{ $count }},@endforeach],
        backgroundColor: '#179e36'
      }],
      labels: [@foreach($sad_cultures_count_filters['type'] as $type => $count)'{{ $type }}',@endforeach]
    },
    options: {
      legend: {
        display: false
      },
      scales: {
        xAxes: [{ticks: {beginAtZero:true}}]
      }
    },
  });
  sad_cultures_chart_bar_morph_chart = new Chart(sad_cultures_chart_bar_morph_ctx, {
    type: 'horizontalBar',
    data: {
      datasets: [{
        data: [@foreach($sad_cultures_count_filters['morph'] as $count){{ $count }},@endforeach],
        backgroundColor: '#179e36'
      }],
      labels: [@foreach($sad_cultures_count_filters['morph'] as $type => $count)'{{ $type }}',@endforeach]
    },
    options: {
      legend: {
        display: false
      },
      scales: {
        xAxes: [{ticks: {beginAtZero:true}}]
      }
    },
  });
  /* End sad statistics */

});
</script>
@endsection