@extends('admin.main')

@section('form')
<div class="box">
  <div class="box-body">
    <table id="modelTable" class="table">
      <thead>
        <tr>
          <th class="">
            Все культуры
          </th>
          <th class="">
            Все сорта
          </th>
          <th class="text-center">
            Количество анкет
          </th>
          <th class="text-center">
            <p>Урожайность на ед. измерения</p>
            <p>
              <span style="font-weight: normal;">ед. измерения:</span>
              <span style="color: gray; font-weight: normal;">мин</span>
              <b>сред</b>
              <span style="color: gray; font-weight: normal;">макс</span>
            </p>
          </th>
          <th class="text-center">
            <p>Урожайность сорта</p>
            <p>
              <span style="color: gray; font-weight: normal;">минимум</span>
              <b>среднее</b>
              <span style="color: gray; font-weight: normal;">максимум</span>
            </p>
          </th>
        </tr>
      </thead>
      <tbody>
      @foreach($sorts as $sort)
        <tr>
          <td class="col-md-2 col-sm-2 col-xs-2">
            {{ $cultures[$sort["culture_id"]]["name"] }}
          </td>
          <td class="col-md-2 col-sm-2 col-xs-2">
              {{ $sort["name"] }}
          </td>
          <td class="col-md-2 col-sm-2 col-xs-2 text-center">
              {{ $questionaries_count[$sort["id"]] }}
          </td>
          <td class="col-md-3 col-sm-3 col-xs-3 text-center" style="word-spacing: 1em;">
            @if(count($harvest_from_one_landing[$sort["id"]]) != 0)
            @foreach($harvest_from_one_landing[$sort["id"]] as $landing_type => $landing_harvest)
            <p>
              <span>{{ $landing_type }}:</span>
              <span style="color: gray;">{{ $landing_harvest["min"] }}</span>
              <b>{{ $landing_harvest["avg"] }}</b>
              <span style="color: gray;">{{ $landing_harvest["max"] }}</span>
            </p>
            @endforeach
            @else
            <p>
              <span style="color: gray;">0</span>
              <b>0</b>
              <span style="color: gray;">0</span>
            </p>
            @endif
          </td>
          <td class="col-md-3 col-sm-3 col-xs-3 text-center" style="word-spacing: 1em;">
            <span style="color: gray;">{{ $harvest_from_all_landing[$sort["id"]]["min"] }}</span>
            <b>{{ $harvest_from_all_landing[$sort["id"]]["avg"] }}</b>
            <span style="color: gray;">{{ $harvest_from_all_landing[$sort["id"]]["max"] }}</span>
          </td>
        </tr>
      @endforeach
      </tbody>
      <tfoot>
        <tr>
          <th class="">
            Все культуры
          </th>
          <th class="">
            Все сорта
          </th>
          <th class="text-center">
            Количество анкет
          </th>
          <th class="text-center">
            <p>Урожайность на ед. измерения</p>
            <p>
              <span style="font-weight: normal;">ед. измерения:</span>
              <span style="color: gray; font-weight: normal;">мин</span>
              <b>сред</b>
              <span style="color: gray; font-weight: normal;">макс</span>
            </p>
          </th>
          <th class="text-center">
            <p>Урожайность сорта</p>
            <p>
              <span style="color: gray; font-weight: normal;">минимум</span>
              <b>среднее</b>
              <span style="color: gray; font-weight: normal;">максимум</span>
            </p>
          </th>
        </tr>
      </tfoot>
    </table>
  </div>
</div>

<hr>

<h3>Построение статистических диаграмм</h3>
<div class="row">
  <div class="col-md-3 col-sm-3 col-xs-12">
    <div class="info-box">
      <span class="info-box-icon" style="background-color: #3c8dbc; color: white;"><i class="fa fa-calendar"></i></span>
      <div class="info-box-content">
        <input type="text" class="form-control" placeholder="От" id="from" name="from" autocomplete="off">
        <input type="text" class="form-control" placeholder="До" id="to" name="to" autocomplete="off">
      </div>
    </div>
  </div>
  <div class="col-md-3 col-sm-3 col-xs-12">
    <div class="info-box">
      <span class="info-box-icon" style="background-color: #a95f8a;">
        <img src="data:image/png;base64," alt="Клумба">
      </span>
      <div class="info-box-content">
        <select class="form-control select2" data-placeholder="Культуры клумбы" id="cultures_klumba" name="cultures_klumba[]" autocomplete="off" multiple="true">
          @foreach($klumba_cultures as $culture)
          <option value="{{ $culture["id"] }}" section="{{ $culture["section_id"] }}">{{ $culture["name"] }}</option>
          @endforeach
        </select>
      </div>
    </div>
  </div>
  <div class="col-md-3 col-sm-3 col-xs-12">
    <div class="info-box">
      <span class="info-box-icon" style="background-color: #f39a24;">
        <img src="data:image/png;base64," alt="Огород">
      </span>
      <div class="info-box-content">
        <select class="form-control select2" data-placeholder="Культуры огорода" id="cultures_ogorod" name="cultures_ogorod[]" autocomplete="off" multiple="true">
          @foreach($ogorod_cultures as $culture)
          <option value="{{ $culture["id"] }}" section="{{ $culture["section_id"] }}">{{ $culture["name"] }}</option>
          @endforeach
        </select>
      </div>
    </div>
  </div>
    <div class="col-md-3 col-sm-3 col-xs-12">
    <div class="info-box">
      <span class="info-box-icon" style="background-color: #179e36;">
        <img src="data:image/png;base64," alt="Сад">
      </span>
      <div class="info-box-content">
        <select class="form-control select2" data-placeholder="Культуры сада" id="cultures_sad" name="cultures_sad[]" autocomplete="off" multiple="true">
          @foreach($sad_cultures as $culture)
          <option value="{{ $culture["id"] }}" section="{{ $culture["section_id"] }}">{{ $culture["name"] }}</option>
          @endforeach
        </select>
      </div>
    </div>
  </div>
</div>

<div class="row">
  <div class="col-md-12 col-sm-12 col-xs-12">
    <div class="info-box">
      <span class="info-box-icon" style="background-color: #3c8dbc; color: white;"><i class="fa fa-line-chart"></i></span>
      <div class="info-box-content">
        <select class="form-control select2" data-placeholder="Статистика" id="statistics" name="statistics[]" autocomplete="off" multiple style="width: 100%;">
          <option value="seeding_date">Посадка</option>
          <option value="ground_transplantation_date">Пересадка</option>
          <option value="trimming_date">Обрезка</option>
          <option value="is_ill">Заболеваемость</option>
          <option value="artificial_irrigation">Искуственный полив</option>
          <option value="drip_irrigation">Капельный полив</option>
          <option value="precipitation_from_planting">Количество осадков с момента посадки</option>
          <option value="feeding_from_planting">Количество подкормок с момента посадки</option>
          <option value="artificial_irrigation_from_planting">Количество искуственного полива с момента поcадки</option>
          <option value="harvest">Полученный сумарный урожай</option>
        </select>
        <input type="button" class="form-control btn btn-primary btn-flat" id="get_statistics_btn" value="Подсчитать">
      </div>
    </div>
  </div>
</div>

@foreach([
  'seeding_date' => 'Количество посадок',
  'ground_transplantation_date' => 'Количество пересадок',
  'trimming_date' => 'Количество обрезок',
  'is_ill' => 'Заболеваемость',
  'artificial_irrigation' => 'Искуственный полив',
  'drip_irrigation' => 'Капельный полив',
  'precipitation_from_planting' => 'Количество осадков с момента посадки',
  'feeding_from_planting' => 'Количество подкормок с момента посадки',
  'artificial_irrigation_from_planting' => 'Количество искуственного полива с момента поcадки',
  'harvest' => 'Полученный сумарный урожай'
] as $statistic => $title)
<div class="row hidden" id="{{ $statistic }}_row">
  <div class="col-md-12 col-sm-12 col-xs-12">
    <div class="box">
      <div class="box-header with-border text-center">
        <h3 class="box-title">{{ $title }}</h3>
      </div>
      <div class="box-body">
        <div class="chart">
          <canvas id="questionaries_chart_line_{{ $statistic }}" width="300" height="75"></canvas>
        </div>
      </div>
    </div>
  </div>
</div>
@endforeach

@endsection

@section('script')
@parent
<script src="{{ asset('adminlte/asset/moment-with-locales.min.js') }}"></script>
<script src="{{ asset('adminlte/asset/Chart.min.js') }}"></script>
<script type="text/javascript">
var table = $('#modelTable').dataTable({
  initComplete: function () {
    this.api().columns([0, 1]).every( function (i) {
      var column = this; 

      if(column.header().attributes[4].name == 'style') {
        var header = column.header().attributes[3].nodeValue;
      } else {
        var header = column.header().attributes[4].nodeValue;
      }
      
      var select = $('<select class="form-control"><option selected="selected" value="">' + header + '</option></select>')
        .appendTo( $(column.header()).empty() )
        .on( 'change', function () {
          var val = $.fn.dataTable.util.escapeRegex(
            $(this).val()
          );

          column
            .search( val ? '^'+val+'$' : '', true, false )
            .draw();
        });

        column.data().unique().sort().each( function ( d, j ) {
          if(column.search() === '^'+d+'$'){
            select.append( '<option value="'+d+'" selected="selected">'+d+'</option>' )
          } else {
            select.append( '<option value="'+d+'">'+d+'</option>' )
          }
        });
    });
  },
  "bPaginate": true,
  "lengthMenu": [[10, 25, 50, -1], [10, 25, 50, "Все"]],
  "bSort": true,
  "order": [],
  "columnDefs": [{ "orderable": false, "targets": [0, 1] }],
  "stateSave": false,
  "scrollX": true,
  "autoWidth": false,
  "language": {
    "lengthMenu": "Показать _MENU_ записей на странице",
    "search": "Поиск:",
    "emptyTable": "Записи отсутствуют",
    "info": "Страница _PAGE_ из _PAGES_",
    "paginate": {
        "first": "Первая",
        "last": "Последняя",
        "next": "Следующая",
        "previous": "Предыдущая"
    }
  }
});

table.api().columns.adjust();



moment.locale('ru');

[from, to].forEach(function(date) {
  date.onchange = function() {
    $('#get_statistics_btn').attr('disabled', from.value > to.value ? true : false);
  }
});

$.fn.datepicker.dates['ru'] = {
    days: ["Воскресенье", "Понедельник", "Вторник", "Среда", "Четверг", "Пятница", "Суббота"],
    daysShort: ["Вск", "Пнд", "Втр", "Срд", "Чтв", "Птн", "Сбт"],
    daysMin: ["Вс", "Пн", "Вт", "Ср", "Чт", "Пт", "Сб"],
    months: ["Январь", "Февраль", "Март", "Апрель", "Май", "Июнь", "Июль", "Август", "Сентябрь", "Октябрь", "Ноябрь", "Декабрь"],
    monthsShort: ["Янв", "Фев", "Мар", "Апр", "Май", "Июн", "Июл", "Авг", "Сен", "Окт", "Ноя", "Дек"],
    today: "Сегодня",
    format: 'yyyy-mm-dd',
    weekStart: 1
};
$('#from').datepicker({language: 'ru', autoclose: true});
$('#to').datepicker({language: 'ru', autoclose: true});

$('.select2').select2();

function getRandomColor() {
  var color = '#';
  for (var i = 0; i < 6; i++) color += '0123456789ABCDEF'[Math.floor(Math.random() * 16)];
  return color;
}

function getSelectValues(select) {
  var result = [];
  var options = select && select.options;
  var opt;
  for (var i=0, iLen=options.length; i<iLen; i++) {
    opt = options[i];
    if (opt.selected) {
      result.push(opt.value || opt.text);
    }
  }
  return result;
}

get_statistics_btn.onclick = function() {
  $.ajax({
    url: "/public/admin/statistics/questionaries_ajax",
    data: {
      'from': from.value,
      'to': to.value,
      'cultures_klumba[]': getSelectValues(cultures_klumba),
      'cultures_ogorod[]': getSelectValues(cultures_ogorod),
      'cultures_sad[]': getSelectValues(cultures_sad),
      'statistics[]': getSelectValues(statistics),
    }
  }).done(function(response) {
    console.log(response.cultures, response.result_statistics);

    var colors = {};
    response.cultures.forEach(function(cultureName) {
      colors[cultureName] = getRandomColor();
    });

    ['seeding_date', 'ground_transplantation_date', 'trimming_date', 'is_ill',
     'artificial_irrigation', 'drip_irrigation', 'precipitation_from_planting', 'feeding_from_planting',
     'artificial_irrigation_from_planting', 'harvest'].forEach(function(statistic) {
      if(response.result_statistics[statistic] != null) {
        $("#" + statistic + "_row").removeClass('hidden');
        new Chart($("#questionaries_chart_line_" + statistic), {
          type: 'line',
          data: {
            datasets: Object.entries(response.result_statistics[statistic]).map(function(culture) {
              return {
                data: Object.entries(culture[1]).map(function(obj) {
                  return {x: obj[0], y: obj[1]};
                }),
                label: culture[0],
                fill: false,
                borderColor: colors[culture[0]],
                backgroundColor: colors[culture[0]],
                lineTension: 0
              };
            })
          },
          options: {
            fill: false,
            scales: {
              xAxes: [{
                type: 'time',
                time: {
                  unit: 'day',
                  displayFormats: {
                     'day': 'DD MMM'
                  }
                }
              }],
              yAxes: [{ticks: {beginAtZero:true}}]
            }
          }
        });
      } else {
        $("#" + statistic + "_row").addClass('hidden');
      }
    });
  }).fail(function(error) {
    console.log(error);
  });
}
</script>
@endsection