...
<button class="btn btn-primary btn-block btn-flat" data-toggle="modal" data-target="#creationModal">Добавить заболевание</button>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title" id="modalLabel">Добавление новой болезни</h4>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      {{ Form::open(['action' => "AdminLTEController@createDiseases$section", 'method' => 'post', 'files' => true]) }}
      {{ csrf_field() }}
      <div class="modal-body">
        ...
        <div class="form-group">
          <label for="creationChemicals">Препараты для борьбы с заболеванием</label>
      ...
      <div class="modal-footer">
        ...
        {{ Form::submit('Добавить заболевание', ['class' => 'btn btn-primary']) }}
      </div>
      {{ Form::close() }}
    </div>
  </div>
    ...
    <table id="modelTable" class="table">
      ...
      @foreach($rows as $row)
        <tr>
          ...
          <td class="col-sm-1 text-center">
            <button class="btn btn-danger" data-toggle="modal" data-target="#delete_disease_{{ $row->id }}" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
            <div class="modal fade" id="delete_disease_{{ $row->id }}" tabindex="-1" role="dialog" aria-labelledby="modalLabel" aria-hidden="true">
              <div class="modal-dialog modal-sm" role="document">
                <div class="modal-content">
                  <div class="modal-header">
                    <h4 class="modal-title" id="modalLabel">Подтверждение удаления</h4>
                  </div>
                  {{ Form::open(['action' => "AdminLTEController@deleteDiseases$section", 'method' => 'delete']) }}
                  {{ csrf_field() }}
                  <input type="text" name="id" value="{{ $row->id }}" class="hidden">
                  <div class="modal-footer">
                    {{ Form::button('Отмена', ['class' => 'btn btn-secondary', 'data-dismiss' => 'modal']) }}
                    {{ Form::submit('Удалить заболевание', ['class' => 'btn btn-danger']) }}
                  </div>
                  {{ Form::close() }}
                </div>
              </div>
    ...
    <div class="modal fade" id="modal_{{ $row->id }}" tabindex="-1" role="dialog" aria-labelledby="modalLabel_{{ $row->id }}" aria-hidden="true" style="overflow-y: scroll;">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          {{ Form::open(['action' => "AdminLTEController@updateDiseases$section", 'method' => 'put', 'files' => true]) }}
          {{ csrf_field() }}
          <input type="text" name="id" value="{{ $row->id }}" class="hidden">
          <div class="modal-body">
            ...
            <table class="table form-group col-sm-12">
              ...
              <tbody>
              @foreach($diseases_photos[$row->id] as $diseases_photo)
                <tr>
                  <td class="col-sm-4">
                    <a target="_blank" rel="noopener noreferrer" href=@php echo($_ENV['PHOTO_FOLDER'].$diseases_photo);@endphp>
                      <img src=@php echo($_ENV['PHOTO_FOLDER'].$diseases_photo);@endphp alt="{{ $diseases_photo }}" class="image col-sm-12" >
                    </a>
                  </td>
                  <td class="col-sm-4">
                    <label for="is_main_{{ $diseases_photo }}" class="hidden"></label>
                    {{ Form::radio('is_main', $diseases_photo, $diseases_photo==$row->main_photo?true:false) }}
                  </td>
                  <td class="col-sm-4">
                    {{ Form::checkbox('for_delete_'.$diseases_photo) }}
                  </td>
                </tr>
              @endforeach
              </tbody>
            ...
            <div class="form-group">
              <label for="diseases_photos_{{ $row->id }}">Загрузить новые фото</label>
              {{ Form::file('diseases_photos_' . $row->id . '[]', ['multiple' => 'multiple']) }}
            </div>
            <hr>
              ...
              <table class="table col-sm-12">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td class="col-sm-4">
                      {{ Form::checkbox('disease_'.$row->id.'_filter_attr_value_'.$filter_attr_value->id) }}
                    </td>
                  </tr>
                  @endforeach
                </tbody>
            ...
            <div class="form-group">
              <label for="disease_cultures">Культуры</label>
              <select class="form-control select2" multiple="multiple" data-placeholder="Выберите культуры"
                      style="width: 100%;" name="disease_cultures[]" id="disease_cultures" autocomplete="off">
                @foreach($cultures as $cultureId => $cultureName)
                <option value="{{ $cultureId }}" @if(isset($disease_cultures[$row->id]) && in_array($cultureId, $disease_cultures[$row->id])) selected="selected" @endif>{{ $cultureName }}</option>
                @endforeach
              </select>
            </div>

            <div class="form-group">
              <label for="disease_sorts">Сорта</label>
              <select class="form-control select2" multiple="multiple" data-placeholder="Выберите сорта"
                      style="width: 100%;" name="disease_sorts[]" id="disease_sorts" autocomplete="off">
                @foreach($sorts as $sort)
                <option value="{{ $sort->id }}" @if(isset($disease_sorts[$row->id]) && in_array($sort->id, $disease_sorts[$row->id])) selected="selected" @endif>{{ $sort->name }}</option>
                @endforeach
              </select>
            </div>

            <div class="form-group">
              <label for="disease_chemicals">Препараты для борьбы с заболеванием</label>
              <select class="form-control select2" multiple="multiple" data-placeholder="Выберите химикаты"
                      style="width: 100%;" name="disease_chemicals[]" id="disease_chemicals" autocomplete="off">
                @foreach($chemicals as $chemical)
                <option value="{{ $chemical->id }}" @if(isset($disease_chemicals[$row->id]) && in_array($chemical->id, $disease_chemicals[$row->id])) selected="selected" @endif>{{ $chemical->name }}</option>
                @endforeach
              </select>
            </div>
...
$(window).ready(function() {
  @if(isset($disease_id))
  var top = document.getElementById("edit_button_{{ $disease_id }}").offsetTop;
...
[@foreach($rows as $row) '{{ $row->id }}', @endforeach].forEach(function(row_id) {
  [@foreach($filter_attributes as $filter_attribute)
   ...
   @endforeach].forEach(function(filter_attr_value_id) {
    $('input[name="disease_'+row_id+'_filter_attr_value_'+filter_attr_value_id+'"]')[0].checked = false;
  });
});
...
@foreach($filter_attr_entities as $entity_filters_key => $entity_filters_value)
  @foreach($filter_attr_entities[$entity_filters_key] as $key => $value)
    $('input[name="disease_{{ $entity_filters_key }}_filter_attr_value_{{ $value }}"]')[0].checked = true;
  @endforeach
@endforeach
